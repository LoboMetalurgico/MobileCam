...
fn main() {
  ...
  let static_frontend_path =
    Path::new(&env::var("OUT_DIR").expect("OUT_DIR not set")).join("frontend.rs");
  let mut static_frontend_file =
    BufWriter::new(File::create(&static_frontend_path).expect("Failed to create output file"));

  let mut frontend_map = Map::new();

  for entry in WalkDir::new(&frontend_dist_dir)
    .into_iter()
    .filter_map(|f| f.ok())
  {
    let path = entry.path();
    ...
    let value = format!(
      ...
      path
        .canonicalize()
        .expect("Cannot canonicalize static file path")
        .display()
    );
  ...
  writeln!(
    static_frontend_file,
    "static FRONTEND: phf::Map<&'static str, &'static [u8]> = {};",
    frontend_map.build()
  )
  .expect("Failed to write output file");
}

fn run_command<C: AsRef<OsStr>, AI: IntoIterator<Item = AS>, AS: AsRef<OsStr>, D: AsRef<Path>>(
  command: C,
  args: AI,
  dir: D,
) {
