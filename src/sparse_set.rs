...
impl<T> SparseSet<T> {
  ...
  /// Returns the number of elements currently stored in the [`SparseSet`].
  ...
  /// Retrieves a mutable reference to a value in the [`SparseSet`] by its index, returning an [`Option<&mut T>`].
  pub fn get_mut(&mut self, index: usize) -> Option<&mut T> {
    if index < self.sparse.len() {
      let dense_index = self.sparse[index];
      if dense_index < self.data.len() && self.dense[dense_index] == index {
        return Some(&mut self.data[dense_index]);
      }
    }
    None
  }
...
impl<T> SyncSparseSet<T> {
  ...
  /// Returns the number of elements currently stored in the [`SyncSparseSet`].
  pub fn len(&self) -> usize {
    self.inner.lock().len()
  }
  ...
  /// Updates a value in the [`SparseSet`] by its index using the provided update function, returning an `Option<U>`.
  pub fn update<U, F: FnOnce(&mut T) -> U>(&self, index: usize, update_fn: F) -> Option<U> {
    self.inner.lock().get_mut(index).map(update_fn)
  }
  ...
  /// Returns a vector of filtered and mapped values from the [`SparseSet`] by applying the provided filter and mapping function to each element.
